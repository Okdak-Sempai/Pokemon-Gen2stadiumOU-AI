...
const logDir = path.resolve(__dirname, '../../../Mewtwo Intelligence');
const logFile = path.join(logDir, 'battle_chat.log');
			...
			options.send = (t: string, data: any) => {
				...
				for (const line of lines) {
					if (line.includes('|move|')) {
						const logLine = `[CHAT][${t}] ${line}\n`;
						fs.appendFileSync(logFile, logLine); // append to file
					}
				}
				this.pushMessage(t, lines.join("\n"));
