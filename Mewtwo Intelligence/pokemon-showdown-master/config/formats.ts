...
import {SampleTeams} from "../data/custom-teams";
...
export const Formats: import('../sim/dex-formats').FormatList = [
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['Standard'],
		banlist: ['UBER', 'Mean Look + Baton Pass'],
		onValidateSet(set) {
			const allowedItems = ['Berry', 'PRZ Cure Berry', 'Ice Berry', 'Mint Berry', 'Bitter Berry', 'Burnt Berry'];
			if (set.item && !allowedItems.includes(set.item)) {
				return [`${set.species || set.name} has an illegal item (${set.item}). Allowed items: ${allowedItems.join(', ')}`];
			}
		},
	},
	{
